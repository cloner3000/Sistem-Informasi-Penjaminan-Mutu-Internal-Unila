<div id="audit-matkul">
    <form action="<?php echo base_url().'audit_ps/save_edit_jawaban'?>" method="post">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="min-width: 20px;max-width: 20px;" class="text-center">
                            <?php foreach($hasil_jawaban as $has):?>
                                <?php if($bo['id_borang'] == $has['borang_id']): ?>
                                    <input value="<?php echo $has['jawaban']?>" name="jawaban_teori[]" id="jawab<?php echo $bo['id_borang'] ?>" class="text-center form-control b-r-lg bg-white" type="text">
                                <?php endif; ?>
                            <?php endforeach;?>  
                        </td>
                        <td style="min-width: 30px;max-width: 30px;" class="text-center">
                            ...
                            <?php foreach($hasil_jawaban as $has):?>
                                <?php if($bo['id_borang'] == $has['borang_id']): ?>
                                    <input value="<?php echo $has['sekor']?>" type="text"  name="skor_teori[]" type="text" id="total<?php echo $bo['id_borang'] ?>" class="subtotal<?php echo $as['id_aspek_penilaian'] ?> text-center form-control b-r-lg bg-white ">
                                <?php endif; ?>
                            <?php endforeach;?> 
                        </td>
                    </tr>
        ...
        <?php if($audit_matkul['sks_prak'] > 0):?>
            <?php $this->view('content/audit/ps/edit_instrument_matakuliah_p');?>
        <?php endif;?>
        <div class="wrapper wrapper-content animated fadeInRight">
            <div>
